...
The package works best when pipes %>% provided by the magrittr package are also
used. 
...
provided by the get_features() function to a dataframe which can then be used 
to plot the schematics. 
...
These have been combined into a geom_chain() function that can be used with 
a pipe in the following way.
...
geom_phospho(p, rel_data, size = 8)
```
