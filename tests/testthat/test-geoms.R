...
# https://stackoverflow.com/questions/13457562/how-to-determine-the-geom-type-of-each-layer-of-a-ggplot2-object/43982598#43982598
  ...
  expect_equal(length(five_rel_data[five_rel_data$type == "DOMAIN",]),
              length(p@layers$geom_rect$data))
})
...
test_that("draw_domains",{
  ...
  expect_equal(class(p@layers$geom_label$geom)[1], "GeomLabel")
...
test_that("draw_phospho",{
  ...
  expect_equal(class(p@layers$geom_point$geom)[1], "GeomPoint")

  # should be 32 phosphorylation sites across data set...
  expect_equal(32, nrow(p$layers$geom_point$data))
...
test_that("draw_motif",{
  ...
  expect_equal(nrow(five_rel_data[five_rel_data$type == "MOTIF",]),
              nrow(p@layers$geom_rect...3$data))
  expect_equal(p@layers$geom_rect...3$data$type[1], "MOTIF" )
})
...
test_that("draw_regions",{
  ...
  expect_equal(p@labels$y, "")
  expect_equal(length(five_rel_data[five_rel_data$type == "DOMAIN",]),
              length(p@layers$geom_rect...3$data))
  # p$layers$geom_rect...3$data contains the data that was extracted
  # dimensions are 6 9
  expect_equal(nrow(five_rel_data[five_rel_data$type == "REGION",]),
              nrow(p@layers$geom_rect...3$data))
  expect_equal(p@layers$geom_rect...3$data$type[1], "REGION" )
})
...
test_that("draw_repeat",{
  ...
  expect_equal(class(p@layers$geom_text$geom)[1], "GeomText")
  ...
  expect_equal(length(five_rel_data[five_rel_data$type == "REPEAT",]),
              length(p@layers$geom_rect$data))
  # p$layers$geom_rect$data contains the data that was extracted
  # dimensions are 6 9
  expect_equal(nrow(five_rel_data[five_rel_data$type == "REPEAT",]),
              nrow(p$layers$geom_rect...3$data))
  expect_equal(p$layers$geom_rect...3$data$type[1], "REPEAT" )
})
...
test_that("draw_recept_dom",{
  ...
  expect_equal(nrow(p$layers$geom_rect...3$data), 4) # 2 receptors - EC and Cyto domain
  expect_equal(nrow(p$layers$geom_rect...4$data), 2) # 2 receptors - 2 TM domain
  ...
  expect_equal(class(p$layers$geom_rect...4$geom)[1], "GeomRect")
  expect_equal(class(p$layers$geom_label$geom)[1], "GeomLabel")
  expect_equal(class(p$layers$geom_label...6$geom)[1], "GeomLabel")

  # in this case nrow of layer 5 should be 2
  # and nrow of layer 6 should be 2
  expect_equal(nrow(p@layers$geom_label$data), 4) # 2 receptors - EC and Cyto domain
  expect_equal(nrow(p@layers$geom_label...6$data), 2) # 2 receptors - 2 TM domain

  # testing data and mapping of layers 5 and 6
  expect_equal(length(p$layers$geom_label$data), 9)
  expect_equal(nrow(p$layers$geom_label$data), 4)
  expect_equal(p$layers$geom_label$data$type[1], "TOPO_DOM")
  expect_equal(p$layers$geom_label$data$description[1], "Extracellular")
  expect_equal(p$layers$geom_label$data$description[2], "Cytoplasmic")
  expect_equal(p$layers$geom_label...6$mapping$label, "TM")
  expect_equal(p$layers$geom_label...6$data$type[1], "TRANSMEM")
  expect_equal(p$layers$geom_label...6$data$description[1], "Helical")
...
test_that("draw_folding",{
  ...
  # in this case nrow of layer 3 should be 4
  # and nrow of layer 4 should be 2
  expect_equal(nrow(p@layers$geom_rect...3$data), 34) # 34 STRAND regions
  expect_equal(nrow(p@layers$geom_rect...4$data), 10) # 10 HELIX regions
  expect_equal(nrow(p@layers$geom_rect...5$data), 1) # 1 TURN
